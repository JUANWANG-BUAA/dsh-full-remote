...
export const Config = Schema.object({
  ...
  commandTimeoutMs: Schema.number().min(1000).default(DEFAULT_COMMAND_TIMEOUT_MS).description('Timeout waiting for the first response byte after a host command POST finishes sending its body: /api/commands/execute (Harness 0.1.2) or a session.prompt whose single text part starts with "/" (Harness 0.1.0/0.1.1, e.g. /compact). Harness command handlers may legitimately run for a long time before responding; the default is 5 minutes and does not affect normal RPC/SSE paths.'),
